// ═══════════════════════════════════════════════════════════════════════════════
// Bach · A quiet window into counterpoint — v5.5
// Fixes: Tone.Convolver crash, robust ZIP parsing, removed fragile Web Worker
// ═══════════════════════════════════════════════════════════════════════════════
...
const getPalette     = () => PALETTES[activePalette] || PALETTES.ensemble;
...
const parseTitle = filename => {
    if (!filename) return { title: 'Untitled', meta: '' };
...
// ═══ Library ══════════════════════════════════════════════════════════════════
// FIX: Path parsing is now robust to any root-folder name, backslashes,
// leading dots/slashes, and different zip creation styles.
class BachLibrary {
    ...
    async load(url) {
        ...
        zip.forEach((rel, entry) => {
            ...
            // ── Robust path normalisation ──────────────────────────────────
            // Handles: backslashes, leading ./ or /, any root-folder name
            const norm  = rel.replace(/\\/g, '/').replace(/^\.?\/?/, '');
            const parts = norm.split('/').filter(p => p.length > 0);
            // Need at least root-folder + filename
            if (parts.length < 2) return;
            // Strip the root folder (index 0), keep the rest as the clean key
            const clean  = parts.slice(1).join('/');
            // The first sub-folder (index 1) becomes the category label;
            // files sitting directly under root go to 'Miscellaneous'
            const folder = parts.length >= 3 ? parts[1] : 'Miscellaneous';
        ...
        // Load all buffers (sequential to avoid memory spikes)
...
class AnalysisEngine {
    ...
    static _chord(activeNotes) {
        ...
        const T = [
            ...
            {n:"sus²",iv:[0,2,7],     p:5 }, {n:"♭II",  iv:[1,5,8],     p:8 },
            {n:"It⁶", iv:[8,0,4],     p:7 }, {n:"Fr⁶",  iv:[8,0,2,6],   p:7 },
            {n:"Ger⁶",iv:[8,0,3,6],   p:7 },
        ];
    ...
    _detectMotifs(notes) {
        ...
        ids.forEach(id => {
            ...
            for (let i = 0; i <= tn.length - LEN; i++) {
                ...
                let match = ctr === key;
                if (!match && this.fugueMode) {
                    const inv = sl.map(n => ({ ...n, midi: sl[0].midi - (n.midi - sl[0].midi) }))
                                  .sort((a, b) => a.time - b.time);
                    const rev = [...sl].reverse();
                    match = contour(inv).join(',') === key || contour(rev).join(',') === key;
                }
                if (match)
    ...
    _buildGhostPaths(entries) {
        ...
        for (let i = 0; i < entries.length - 1; i++) {
            ...
            paths.push({
                startTime: a.endTime, endTime: Math.max(b.time, a.endTime + 0.25),
                fromTrack: a.track, toTrack: b.track,
                fromMidi: a.notes[a.notes.length-1].midi, toMidi: b.notes[0].midi,
    ...
    _buildInsightEvents() {
        ...
        if (this.motifs.length) {
            this.motifs[0].entries.forEach((e, i) => {
                const prev = this.motifs[0].entries[i-1];
    ...
    _detectKeys(notes, totalTime) {
        ...
        const kCol = (pc, mode) => {
            const h = Math.round((F5.indexOf(pc) / 12) * 360);
...
// ═══ AudioEngine ══════════════════════════════════════════════════════════════
// FIX 1: Replaced Tone.Convolver({url}) with Tone.Reverb (generated, no external URL)
//        — Convolver with a URL could throw/fail silently on load, crashing the app
// FIX 2: Removed Web Worker for MIDI parsing — Worker's importScripts can fail
//        (CORS, CSP) causing loadTrack to hang forever as a stalled Promise
class AudioEngine {
    constructor() {
        ...
        // Use generated Tone.Reverb — no external URL, safe constructor
        try {
            this.reverb = new Tone.Reverb({ decay: 2.5, preDelay: 0.01, wet: 0.18 });
            this.reverb.toDestination();
            this.reverb.generate(); // async, non-blocking
        } catch (e) {
            console.warn('Reverb init failed, running dry:', e);
            this.reverb = null;
        }
    ...
    setReverb(on) {
        ...
        if (this.reverb) this.reverb.wet.value = on ? 0.18 : 0;
    }

    // Direct main-thread parse — simple, reliable, no Worker needed
    loadTrack(buf) {
    ...
    async play(startAt, useIntro, onComplete) {
        ...
        const go = () => {
            ...
            this.notes.forEach(n => {
                ...
                const dim = solo.size > 0 && !solo.has(n.track);
                const vel = dim ? 0.03 : 0.5 + (n.velocity - 0.5) * Math.pow(this.dynamicsAmount, 1.5);
...
class Renderer {
    ...
    whiteIndexOf(midi) {
        const oct = Math.floor(midi / 12), s = ((midi % 12) + 12) % 12;
        let idx = 0;
    ...
    getLayout(w, h) {
        const mb = isMobile(), top = mb ? 16 : 24, bottom = mb ? 16 : 24;
    ...
    draw(audioTime, inIntro = false, introProgress = 0) {
        ...
        if (this.totalTime > 0) this._drawTonalBar(now, w, h);
        if (this.view !== 'voice-constellation') this._drawTimeGrid(now, w, top, height, playheadX, pxPerSec);
        ...
        if (this.analysisData?.ready && this.view !== 'voice-constellation') {
            ...
            this._drawInsightAnnotation(now, w, top);
        }

        if (this.view !== 'voice-constellation') this._drawPlayhead(playheadX, top, height);
    ...
    _drawTonalBar(now, w, h) {
        ...
        segs.forEach((s, i) => {
            const x0 = Math.floor((s.time    / total) * w);
            const x1 = Math.floor((s.endTime / total) * w);
    ...
    _drawInsightAnnotation(now, w, top) {
        ...
        const color = insight.type === 'motif'
            ? pal.voices[(insight.track || 0) % pal.voices.length]
            : insight.type === 'key' ? pal.gold : pal.ink;
        this.ctx.save();
        this.ctx.globalAlpha  = ba * fade * 0.6;
        this.ctx.fillStyle    = color;
        this.ctx.font         = `italic ${mb ? 11 : 13}px 'EB Garamond',serif`;
        this.ctx.textAlign    = 'right'; this.ctx.textBaseline = 'top';
        this.ctx.fillText(insight.text, w - (mb ? 14 : 46), top + (mb ? 18 : 24));
        this.ctx.restore();
    }
    ...
    _drawArchitectural(now, w, top, height, playheadX, pxPerSec) {
        ...
        for (const n of this.notesSorted) {
            ...
            this.ctx.fill();
            if (act) {
                this.ctx.strokeStyle = 'rgba(255,248,220,0.45)'; this.ctx.lineWidth = 0.5;
    ...
    _drawVoiceConstellation(now, w, h) {
        ...
        const ba = this.ctx.globalAlpha, pal = getPalette(), ctx = this.ctx, solo = this.soloedTracks;
        ...
        if (chord?.name) {
            const QM = { "":"major","m":"minor","°":"diminished","⁺":"augmented","⁷":"dominant 7th","maj⁷":"major 7th","m⁷":"minor 7th","ø":"half-diminished","°⁷":"diminished 7th","sus⁴":"suspended 4th","sus²":"suspended 2nd" };
            ctx.save();
        ...
        pts.forEach(p => {
            ...
            ctx.beginPath(); ctx.arc(p.x, p.y, 22, 0, Math.PI*2); ctx.fill(); ctx.restore();
    ...
    _drawGhostHandoff(now, w, top, height, playheadX, pxPerSec) {
        ...
        this.analysisData.ghostPaths.forEach(path => {
            ...
            let fade = prog < 0.1 ? prog/0.1 : prog > 0.75 ? Math.max(0, 1-(prog-0.75)/0.25) : 1.0;
            fade *= 0.95;
            if (fade < 0.01) return;
            ...
            ctx.beginPath(); ctx.moveTo(fromX, fromY); ctx.quadraticCurveTo(cpX, cpY, cx, cy); ctx.stroke();
            ctx.shadowBlur = 0;

            // Motif note cluster
            path.noteMidis.forEach((midi, i) => {
                const tFrac = path.duxDuration > 0 ? path.noteTimeOffsets[i] / path.duxDuration : i / (path.noteMidis.length - 1 || 1);
                const dotX  = cx + Math.floor((tFrac - 0.5) * 52);
                const natY  = Math.floor(this.midiToY(midi, top, height));
                const dotY  = Math.floor(natY * (1 - t) + cy * t);
                const isFirst = i === 0;
                let dotAlpha = ba * fade * (isFirst ? 0.72 : 0.45);
                if (t > 0.8) dotAlpha *= 1 - (t - 0.8) / 0.2;
                ctx.globalAlpha = Math.min(1, dotAlpha);
                ctx.fillStyle   = color;
                ctx.shadowBlur  = isFirst ? 6 : 3; ctx.shadowColor = color;
                ctx.beginPath(); ctx.arc(dotX, dotY, isFirst ? 4.5 : 2.8, 0, Math.PI*2); ctx.fill();
            });

            // Arrival ring
            if (t > 0.7) {
                const aF = Math.min(1, (t-0.7)/0.3) * (1 - Math.max(0, (prog-0.9)/0.1));
                if (aF > 0.01) {
                    ctx.shadowBlur = 0; ctx.globalAlpha = ba * aF * 0.5;
                    ctx.strokeStyle = color; ctx.lineWidth = 1.2;
                    ctx.beginPath(); ctx.arc(toX, toY, 12 + (t-0.7)*8, 0, Math.PI*2); ctx.stroke();
                }
            }
            ctx.restore();
        });
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // FIX: wrapped in try-catch so any constructor error shows a helpful message
    // instead of silently hanging on "Preparing the score"
    let ren, engine, lib, analysis;
    try {
        ...
        metaEl.textContent  = initErr.message || 'see console';
        return;
    }
    ...
    // All files for the current filter query
    let allOptions = [];
    ...
    function buildVoiceButtons() {
        ...
        ids.forEach(id => {
            ...
            const avg = ns.length ? ns.reduce((s, n) => s + n.midi, 0) / ns.length : 60;
            const lbl = avg > 72 ? 'S' : avg > 60 ? 'A' : avg > 48 ? 'T' : 'B';
            const btn = document.createElement('button');
            btn.className = 'voice-btn'; btn.dataset.track = id; btn.textContent = lbl;
    ...
    function toggleVoice(id) {
        if (soloedTracks.has(id)) soloedTracks.delete(id); else soloedTracks.add(id);
        updateVoiceBtnUI();
        ren.soloedTracks    = new Set(soloedTracks);
        engine.soloedTracks = new Set(soloedTracks);
        if (engine.isPlaying && !introActive)
            engine.play(Math.max(0, Tone.Transport.seconds), false, () => {});
    ...
    function updateVoiceBtnUI() {
        voiceIso.querySelectorAll('.voice-btn').forEach(btn => {
            ...
            if (!soloedTracks.size) return;
            btn.classList.add(soloedTracks.has(id) ? 'solo' : 'muted');
        });
    }

    // ── Dynamics ──────────────────────────────────────────────────────────────
    if (dynSlider) {
        const sv = localStorage.getItem('bach-dynamics');
        if (sv) dynSlider.value = sv;
        const upd = v => {
            engine.dynamicsAmount = v / 100;
            if (dynLabel) dynLabel.textContent = v<=5?'organ':v<=30?'subtle':v<=60?'expressive':v<=80?'dramatic':'full';
    ...
    // ── Library loading ───────────────────────────────────────────────────────
    try {
        titleEl.textContent = 'Loading library…';
        ...
        titleEl.classList.remove("breathing", "loading");
        metaEl.textContent  = 'place the file in the same folder as index.html';
        tSel.disabled = true; playBtn.disabled = true;
    }
    ...
    function populateSelector(filter) {
        ...
        folders.forEach(([folder, files]) => {
            ...
            files.sort().forEach(file => {
                const title = parseTitle(file).title;
                if (q && !title.toLowerCase().includes(q) && !file.toLowerCase().includes(q)) return;
                const o = document.createElement("option"); o.value = file; o.textContent = title;
                g.appendChild(o); totalShown++;
            });
            if (g.children.length > 0) tSel.appendChild(g);
        });

        if (totalShown === 0) {
            tSel.innerHTML = "<option>No matches</option>";
        } else {
    ...
    // Filter: rebuild options (works cross-browser, unlike option.hidden)
    if (filterInput) {
        filterInput.addEventListener('input', () => populateSelector(filterInput.value));
    }
    ...
    const doPlay = async () => {
        try {
            if (ren.notes.length === 0) {
                ...
                if (!buf) { console.warn('No buffer for:', tSel.value); return; }
                const cacheKey = tSel.value;

                if (analysisCache.has(cacheKey)) {
                    const cached = analysisCache.get(cacheKey);
                    engine.notes = cached.notes; engine.tempo = cached.tempo;
                    ren.setup(cached.notes, cached.dur, cached.tempo);
                    Object.assign(analysis, cached.analysis);
                    analysis.ready = true; ren.analysisData = analysis;
                    buildVoiceButtons();
                } else {
                    const dur = engine.loadTrack(buf);
                    ren.setup(engine.notes, dur, engine.tempo);
                    buildVoiceButtons();
                    // Run analysis after a frame so UI doesn't freeze
                    await new Promise(res => setTimeout(res, 0));
                    analysis.analyze(engine.notes, dur, engine.tempo);
                    ren.analysisData = analysis;
                    try { analysisCache.set(cacheKey, { notes: engine.notes, dur, tempo: engine.tempo, analysis: { ...analysis } }); } catch(_){}
                }
            }
            await Tone.loaded();
    ...
    const onSeekEnd = async e => {
        ...
        if (wasPlaying) {
            ...
            await engine.play(newT, false, () => {}); ren.isAnimating = true; startLoop();
    ...
    if (window.matchMedia("(hover:hover)").matches) {
        canvas.addEventListener("mousemove", e => {
            ...
            const note = ren.noteAt(e.clientX - rect.left, e.clientY - rect.top, introActive ? 0 : Math.max(0, Tone.Transport.seconds), canvas.clientWidth, canvas.clientHeight);
    ...
    document.addEventListener("keydown", e => {
        ...
        if (e.code === "Space") { e.preventDefault(); if (!playBtn.disabled) playBtn.click(); }
            ...
            if (engine.isPlaying && !introActive) engine.play(newT, false, () => {}); else ren.draw(newT, false);
