...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (dynamicsSlider) {
        ...
        const updateDynamicsLabel = (val) => {
            const pct = Math.round(val);
            let desc;
            if (pct <= 5)        desc = "organ";
            else if (pct <= 30)  desc = "subtle";
            else if (pct <= 60)  desc = "expressive";
            else if (pct <= 80)  desc = "dramatic";
            else                 desc = "full";
            if (dynamicsLabel) dynamicsLabel.textContent = desc;
        };
        updateDynamicsLabel(dynamicsSlider.value);
        dynamicsSlider.addEventListener("input", (e) => {
            engine.dynamicsAmount = e.target.value / 100;
            updateDynamicsLabel(e.target.value);
        });
    }
